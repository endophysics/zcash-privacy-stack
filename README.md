...
This command is intentional, local inspection evidence rather than a production logging mode. Its stable transcript excludes transaction and admission identifiers. `PATH`, the resolved local `just` executable, `TEST_ZCASHD_PATH`, and the inherited process environment are trusted operator inputs; do not run the command in an attacker-controlled CI environment. No transaction identifiers are added to default production logs.
